...
export default function ShipmentTable({
  ...
  const [shipmentForm, setShipmentForm] = useState<IShipmentForm>({
    cost: '',
    description: '',
    orderNumber: '',
  })
      ...
      <div className="ShipmentTable__formContainer">
        ...
        <form className="WorkspaceList__form" action="" onSubmit={handleAddShipment}>
          <div className="WorkspaceList__form-fields">
            {shipmentFormFieldsArr?.map((shipmentInput: IShipmentInput) => (
              <ShipmentInput
                key={shipmentInput.keyStr}
                form={shipmentForm}
                setForm={setShipmentForm}
                name={shipmentInput.name}
                keyStr={shipmentInput.keyStr}
                type={shipmentInput.type}
                pattern={shipmentInput.pattern}
                tooltip={shipmentInput.tooltip}
              />
            ))}
          </div>
          <button className="WorkspaceList__submit" type="submit">
            Add Shipment
          </button>
        </form>
      </div>
